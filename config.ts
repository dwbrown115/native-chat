export const FIREBASE_API_KEY = `${process.env.EXPO_FIREBASE_API_KEY}`;
export const FIREBASE_AUTH_DOMAIN = `${
  process.env.EXPO_FIREBASE_AUTH_DOMAIN
}`;
export const FIREBASE_APP_ID = `${process.env.EXPO_FIREBASE_APP_ID}`;
export const FIREBASE_PROJECT_ID = `${
  process.env.EXPO_FIREBASE_PROJECT_ID
}`;
export const FIREBASE_STORAGE_BUCKET = `${
  process.env.EXPO_FIREBASE_STORAGE_BUCKET
}`;
export const FIREBASE_MEASUREMENT_ID = `${
  process.env.EXPO_FIREBASE_MEASUREMENT_ID
}`;
export const FIREBASE_MESSAGING_SENDER_ID = `${
  process.env.EXPO_FIREBASE_MESSAGING_SENDER_ID
}`;
export const ALGOLIA_APPLICATION_ID = `${
  process.env.EXPO_ALGOLIA_APPLICATION_ID
}`;
export const ALGOLIA_SEARCH_KEY = `${process.env.EXPO_ALGOLIA_SEARCH_KEY}`;
