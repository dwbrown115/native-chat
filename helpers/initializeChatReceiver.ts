...
export default async function initializeChatReciver(chatRoomId: string) {
  ...
  const receiverPublicKey = { receiverPublicKey: keyPair.publicKey };
  await updateData("ChatRooms", chatRoomId, receiverPublicKey);
